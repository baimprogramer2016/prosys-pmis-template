...
use App\Models\ReportDailyHistory;
...
class ReportDailyController extends Controller
{
    public function index(Request $request){
        try{
            return view('pages.report-daily.report');
        }catch (Throwable $e) {
            // Tangani error
    ...
    public function getReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            ->addColumn('action', function($row) {
                ...
                $editBtn = '';
                if (Gate::allows('edit_daily_report')) {
                    $editBtn = '<a class="dropdown-item" href="'.route('report-daily-edit', $row->id).'">Edit</a>';
                }
            
                // Tombol Delete (Hanya tampil jika user memiliki izin 'delete_schedule')
                $deleteBtn = '';
                if (Gate::allows('delete_daily_report')) {
    ...
    public function tambah(Request $request){
        try{
            ...
            return view('pages.report-daily.report-tambah');
        }catch (Throwable $e) {
            // Tangani error
            return response()->json([
                'message' => 'Terjadi kesalahan saat menyimpan data.',
    ...
    public function saveUploads(Request $request)
    {
        ...
        foreach ($uploadedFiles as $file) {
            ...
            $newPath = str_replace('temp', 'public/reportdaily', $file['path']);
    ...
    public function viewEdit(Request $request, $id){
        ...
        try{
            $document = ReportDaily::find($id);
        
          return view('pages.report-daily.report-edit',[
            "document" => $document
          ]);
    ...
    public function update(Request $request, $id)
    {
        ...
        $doc = ReportDaily::find($id);

            //insert ke history
            $docHistory = new ReportDailyHistory();
            $docHistory->report_daily_id = $doc->id;
        ...
        if (!empty($uploadedFiles) && is_array($uploadedFiles)) {
            ...
            $newPath = str_replace('temp', 'public/reportdaily', $uploadedFiles[0]['path']);
    ...
    public function viewDelete(Request $request, $id){
        ...
        try{
            $document = ReportDaily::find($id);
            return view('pages.report-daily.report-delete', [
                "document" => $document,
            ]);
        }catch (Throwable $e) {
            // Tangani error
    ...
    public function share(Request $request, $id){
        ...
        try{
            $document = ReportDaily::find($id);
            return view('pages.report-daily.report-share', [
                "document" => $document,
            ]);
        }catch (Throwable $e) {
            // Tangani error
    ...
    public function pdf(Request $request, $id){ 
        try{
            $document = ReportDaily::find($id);
            return view('pages.report-daily.report-pdf', [
                "document" => $document,
            ]);
        }catch (Throwable $e) {
            // Tangani error
    ...
    public function history(Request $request, $id){ 
        try{
            $document = ReportDailyHistory::where('report_daily_id', $id)->get();
