...
class RfiController extends Controller
{
    ...
    public function getRfi(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            ->addColumn('action', function($row) {
                ...
                $editBtn = '';
                if (Gate::allows('edit_rfi')) {
                    $editBtn = '<a class="dropdown-item" href="'.route('rfi-edit', $row->id).'">Edit</a>';
                }
            
                // Tombol Delete (Hanya tampil jika user memiliki izin 'delete_schedule')
                $deleteBtn = '';
                if (Gate::allows('delete_rfi')) {
    ...
    public function tambah(Request $request){
        try{
            ...
            return view('pages.rfi.rfi-tambah');
        }catch (Throwable $e) {
            // Tangani error
            return response()->json([
                'message' => 'Terjadi kesalahan saat menyimpan data.',
    ...
    public function saveUploads(Request $request)
    {
        ...
        foreach ($uploadedFiles as $file) {
            ...
            $newPath = str_replace('temp', 'public/rfi', $file['path']);
    ...
    public function viewEdit(Request $request, $id){
        ...
        try{
            $document = Rfi::find($id);
        
          return view('pages.rfi.rfi-edit',[
            "document" => $document
          ]);
    ...
    public function update(Request $request, $id)
    {
        ...
        $doc = Rfi::find($id);
            ...
            //insert ke history
            $docHistory = new RfiHistory();
            $docHistory->rfi_id = $doc->id;
        ...
        if (!empty($uploadedFiles) && is_array($uploadedFiles)) {
            ...
            $newPath = str_replace('temp', 'public/rfi', $uploadedFiles[0]['path']);
    ...
    public function viewDelete(Request $request, $id){
        ...
        try{
            $document = Rfi::find($id);
            return view('pages.rfi.rfi-delete', [
                "document" => $document,
            ]);
        }catch (Throwable $e) {
            // Tangani error
    ...
    public function share(Request $request, $id){
        ...
        try{
            $document = Rfi::find($id);
            return view('pages.rfi.rfi-share', [
                "document" => $document,
            ]);
        }catch (Throwable $e) {
            // Tangani error
    ...
    public function history(Request $request, $id){ 
        try{
            $document = RfiHistory::where('rfi_id', $id)->get();
            return view('pages.rfi.rfi-history', [
                "documents" => $document,
            ]);
        }catch (Throwable $e) {
            // Tangani error
