        ...
        if ($request->ajax()) {
            ...
            ->addColumn('action', function($row) use($request){
                ...
                if(in_array($row->ext,['pdf','jpg','png','jpeg','docx','doc','xls','xlsx','ppt','pptx'])){
                  ...
                  $editBtn = '';
                  if (Gate::allows('edit_personnel_hr')) {
                      $editBtn = ' <a class="dropdown-item" href="' . route('custom-personnel-hr-edit', ['id' => $row->id, 'tab' => $request->tab]) . '">Edit</a>';
                  }
              
                  // Tombol Delete (Hanya tampil jika user memiliki izin 'delete_schedule')
                  $deleteBtn = '';
                  if (Gate::allows('delete_personnel_hr')) {
