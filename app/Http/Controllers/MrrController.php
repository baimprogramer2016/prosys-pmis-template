...
class MrrController extends Controller
{
    ...
    public function getMrr(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            ->addColumn('action', function($row) {
                ...
                $editBtn = '';
                if (Gate::allows('edit_mrr')) {
                    $editBtn = '<a class="dropdown-item" href="'.route('mrr-edit', $row->id).'">Edit</a>';
                }
            
                // Tombol Delete (Hanya tampil jika user memiliki izin 'delete_schedule')
                $deleteBtn = '';
                if (Gate::allows('delete_mrr')) {
    ...
    public function tambah(Request $request){
        try{
            ...
            return view('pages.mrr.mrr-tambah');
        }catch (Throwable $e) {
            // Tangani error
            return response()->json([
                'message' => 'Terjadi kesalahan saat menyimpan data.',
    ...
    public function saveUploads(Request $request)
    {
        ...
        foreach ($uploadedFiles as $file) {
            ...
            $newPath = str_replace('temp', 'public/mrr', $file['path']);
    ...
    public function viewEdit(Request $request, $id){
        ...
        try{
            $document =Mrr::find($id);
        
          return view('pages.mrr.mrr-edit',[
            "document" => $document
          ]);
    ...
    public function update(Request $request, $id)
    {
        ...
        $doc =Mrr::find($id);
            ...
            //insert ke history
            $docHistory = new MrrHistory();
            $docHistory->mrr_id = $doc->id;
        ...
        if (!empty($uploadedFiles) && is_array($uploadedFiles)) {
            ...
            $newPath = str_replace('temp', 'public/mrr', $uploadedFiles[0]['path']);
    ...
    public function viewDelete(Request $request, $id){
        ...
        try{
            $document =Mrr::find($id);
            return view('pages.mrr.mrr-delete', [
                "document" => $document,
            ]);
        }catch (Throwable $e) {
            // Tangani error
    ...
    public function share(Request $request, $id){
        ...
        try{
            $document =Mrr::find($id);
            return view('pages.mrr.mrr-share', [
                "document" => $document,
            ]);
        }catch (Throwable $e) {
            // Tangani error
    ...
    public function history(Request $request, $id){ 
        try{
            $document = mrrHistory::where('mrr_id', $id)->get();
            return view('pages.mrr.mrr-history', [
                "documents" => $document,
            ]);
        }catch (Throwable $e) {
            // Tangani error
