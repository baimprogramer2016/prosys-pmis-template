    ...
    <div class="row">
      ...
      <div class="card ">   
        <div class="card-body">
          ...
          <form action="{{ route('custom-quality-management-upload-temp')}}" class="dropzone mt-3" id="myDropzone">
...
document.getElementById('saveUploads').addEventListener('click', function () {
  ...
  if(valid == true){
    ...
    $.ajax({
      url: "{{ route('custom-quality-management-update-uploads',':id') }}".replace(':id', id_edit),
      type: "POST",
      data: {
        _token: "{{ csrf_token() }}",
