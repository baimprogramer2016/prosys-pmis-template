...
<div class="page-inner">
    ...
    >
      <div class="d-flex align-items-center gap-4">
        ...
        <h6 class="op-7 mb-2">Quality Management / {{ Ucwords(str_replace('_',' ',request('tab'))) }} </h6>
       
        </div> 
     
      <div class="ms-md-auto py-2 py-md-0">
        @can('add_quality_management')
        <a href="{{ route('custom-quality-management-tambah',["tab" => request('tab')]) }}"  class="btn btn-primary btn-round">Tambah</a>
        @endcan
       </div>
    </div>
    <div class="row">
...
function viewDelete(param){
  ...
  $(".modal-content").html("");
$.ajax({
  url: "{{ route('custom-quality-management-delete', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
    type: "GET",
    data: {
        tab:tab 
      },
...
function viewShare(param){
  ...
  $(".modal-content").html("");
$.ajax({
  url: "{{ route('custom-quality-management-share', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
    type: "GET",
    data: {
        tab:tab 
      },
...
function viewPdf(param){
  ...
  $(".modal-content").html("");
  $.ajax({
    url: "{{ route('custom-quality-management-pdf', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
      type: "GET",
      data: {
        tab:tab 
      },
 ...
 $(document).ready(function() {
  var table = $('.table').DataTable({
          ...
          language : {
            ...
            ajax: {
                url: "{{ route('get-custom-quality-management', ['tab' => request('tab')]) }}",
            },
...
function viewHistory(param){
  ...
  $(".modal-content").html("");
  $.ajax({
  url: "{{ route('custom-quality-management-history', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
    type: "GET",
    data: {
        tab:tab 
    },
