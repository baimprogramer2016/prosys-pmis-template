...
<div class="page-inner">
    ...
    >
      <div class="d-flex align-items-center gap-4">
        ...
        <h6 class="op-7 mb-2">Schedule Management</h6>
       
        </div> 
     
      <div class="ms-md-auto py-2 py-md-0">
        @can('add_doc_schedule')
...
function viewDelete(param){
  $(".modal-content").html("");
$.ajax({
  url: "{{ route('schedule-management-delete', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
    type: "GET",
    headers: {
...
function viewShare(param){
  $(".modal-content").html("");
$.ajax({
  url: "{{ route('schedule-management-share', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
    type: "GET",
    headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    },
...
function viewPdf(param){
  $(".modal-content").html("");
  $.ajax({
    url: "{{ route('schedule-management-pdf', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
      type: "GET",
      headers: {
          'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
      },
 ...
 $(document).ready(function() {
  var table = $('.table').DataTable({
          ...
          ajax: {
            ...
            url : "{{ route('get-schedule-management') }}",
          },
