...
<div class="page-inner">
    ...
    >
      <div class="d-flex align-items-center gap-4">
        ...
        <h6 class="op-7 mb-2">Field Instruction / Approve</h6>
...
function viewModal(param){
  $(".modal-content").html("");
$.ajax({
  url: "{{ route('field-instruction-approve-modal', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
    type: "GET",
    headers: {
...
function viewDelete(param){
  $(".modal-content").html("");
$.ajax({
  url: "{{ route('field-instruction-delete', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
    type: "GET",
    headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    },
...
function viewShare(param){
  $(".modal-content").html("");
$.ajax({
  url: "{{ route('field-instruction-share', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
    type: "GET",
    headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    },
...
function viewPdf(param){
  $(".modal-content").html("");
  $.ajax({
    url: "{{ route('field-instruction-pdf', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
      type: "GET",
      headers: {
          'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
      },
...
function viewHistory(param){
  $(".modal-content").html("");
$.ajax({
  url: "{{ route('field-instruction-history', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
    type: "GET",
    headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    },
 ...
 $(document).ready(function() {
  var table = $('.table').DataTable({
          ...
          ajax: {
            url : "{{ route('get-field-instruction',['field' => 'verifikasi', 'status' => 'review']) }}",
          },
