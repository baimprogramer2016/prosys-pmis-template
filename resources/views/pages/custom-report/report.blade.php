...
<div class="page-inner">
    ...
    >
      ...
      <div class="ms-md-auto py-2 py-md-0">
        @can('add_custom_report')
        <a href="{{ route('custom-report-tambah',["tab" => request('tab')]) }}"  class="btn btn-primary btn-round">Tambah</a>
        @endcan
       </div>
    </div>
    <div class="row">
            ...
            <table class="table table-bordered" id="myTable">
              <thead>
                <tr>  
                  ...
                  <th  class="bg-th">Type Of Report</th>
...
function viewDelete(param){
  ...
  $(".modal-content").html("");
$.ajax({
  url: "{{ route('custom-report-delete', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
    type: "GET",
    data: {
        tab:tab 
      },
...
function viewShare(param){
  ...
  $(".modal-content").html("");
$.ajax({
  url: "{{ route('custom-report-share', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
    type: "GET",
    data: {
        tab:tab 
      },
...
function viewPdf(param){
  ...
  $(".modal-content").html("");
  $.ajax({
    url: "{{ route('custom-report-pdf', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
      type: "GET",
      data: {
        tab:tab 
      },
 ...
 $(document).ready(function() {
  var table = $('.table').DataTable({
          ...
          language : {
            ...
            ajax: {
                url: "{{ route('get-custom-report', ['tab' => request('tab')]) }}",
            },
...
function viewHistory(param){
  ...
  $(".modal-content").html("");
  $.ajax({
  url: "{{ route('custom-report-history', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
    type: "GET",
    data: {
        tab:tab 
    },
