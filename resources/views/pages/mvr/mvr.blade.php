...
<div class="page-inner">
    ...
    >
      <div class="d-flex align-items-center gap-4">
        ...
        <h6 class="op-7 mb-2">Report / Material Verification Report</h6>
       
        </div> 
     
      <div class="ms-md-auto py-2 py-md-0">
        <a href="{{ route('mvr-tambah') }}"  class="btn btn-primary btn-round">Tambah</a>
       </div>
    </div>
    <div class="row">
              ...
              <div class="card-body">
                <div class="row align-items-center">
                  ...
                  <div class="col col-stats ms-3 ms-sm-0 d-flex">
                    <div class="numbers">
                      <h4 class="card-title">Material Verification Report</h4>
                    </div>
...
function viewDelete(param){
  $(".modal-content").html("");
$.ajax({
  url: "{{ route('mvr-delete', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
    type: "GET",
    headers: {
...
function viewShare(param){
  $(".modal-content").html("");
$.ajax({
  url: "{{ route('mvr-share', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
    type: "GET",
    headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    },
...
function viewPdf(param){
  $(".modal-content").html("");
  $.ajax({
    url: "{{ route('mvr-pdf', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
      type: "GET",
      headers: {
          'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
      },
...
function viewHistory(param){
  $(".modal-content").html("");
$.ajax({
  url: "{{ route('mvr-history', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
    type: "GET",
    headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    },
