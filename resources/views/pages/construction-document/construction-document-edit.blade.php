    ...
    <div class="page-inner">
        <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row  pb-2">
            ...
            <div class="ms-md-auto py-2 py-md-0">
                <a href="{{ route('construction-document') }}" class="btn btn-primary btn-round">Daftar</a>
            </div>
        </div>
        <div class="row">
                        ...
                        <div class="row mt-4">
                            <div class="col-md-3">
                                <div class="alert-warning text-center">Hanya bisa Upload 1 File <strong>Kosongkan jika
                                        Document tidak dirubah</strong></div>
                                <form action="{{ route('construction-document-upload-temp') }}" class="dropzone mt-3"
                                    id="myDropzone">
                            ...
                            <div class="col-md-9">
                                ...
                                <div class=" mb-3">
                                    ...
                                    <textarea class="form-control" id="document_number" name="document_number">
                                        {{ $document->document_number }}</textarea>
                                </div>
                                <div class=" mb-3">
                                ...
                                <div class=" mb-3">
                                    ...
                                    <input type="date" class="form-control" id="tanggal" name="tanggal"
                                        value="{{ \Carbon\Carbon::parse($document->tanggal)->format('Y-m-d') }}">
                                </div>
                                <div class=" mb-3">
                                    <label for="start_date" class="form-label strong">Discipline</label>
                                    <select class="form-select" id="discipline" name="discipline">
                                        <option value="{{ $document->discipline }}">
                                            {{ optional($document->r_discipline)->description }} </option>
                                        <option value=""">---Pilih---</option>
                                        @foreach ($data_discipline as $item_discipline)
                                            <option value="{{ $item_discipline->id }}">
                                                {{ $item_discipline->description }} </option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class=" mb-3">
                                    ...
                                    <select class="form-select" id="category" name="category">
                                        <option value="{{ $document->category }}">
                                            {{ optional($document->r_category)->description }} </option>
                                        {{-- <option value="">---Pilih---</option>
...
@push('bottom')
    ...
    <script>
        ...
        document.getElementById('saveUploads').addEventListener('click', function() {
            ...
            if (valid == true) {
                $.ajax({
                    url: "{{ route('construction-document-update-uploads', ':id') }}".replace(':id',
                        id_edit),
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
