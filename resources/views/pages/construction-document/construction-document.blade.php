...
<div class="page-inner">
    ...
    >
      <div class="d-flex align-items-center gap-4">
        ...
        <h6 class="op-7 mb-2">Construcrion Document</h6>
          ...
          <div class="card-body">
            <div class="row align-items-center">
              ...
              <div class="col col-stats ms-3 ms-sm-0 d-flex">
                <div class="numbers">
                  <h4 class="card-title">Construction Document</h4>
                </div>
...
function viewModal(param){
  $(".modal-content").html("");
$.ajax({
  url: "{{ route('construction-document-approve-modal', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
    type: "GET",
    headers: {
...
function viewDelete(param){
  $(".modal-content").html("");
$.ajax({
  url: "{{ route('construction-document-delete', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
    type: "GET",
    headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    },
...
function viewShare(param){
  $(".modal-content").html("");
$.ajax({
  url: "{{ route('construction-document-share', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
    type: "GET",
    headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    },
...
function viewPdf(param){
  $(".modal-content").html("");
  $.ajax({
    url: "{{ route('construction-document-pdf', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
      type: "GET",
      headers: {
          'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
      },
...
function viewHistory(param){
  $(".modal-content").html("");
$.ajax({
  url: "{{ route('construction-document-history', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
    type: "GET",
    headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    },
 ...
 $(document).ready(function() {
  var table = $('.table').DataTable({
          ...
          ajax: {
            ...
            url : "{{ route('get-construction-document',['field' => 'mdr', 'status' => '0']) }}",
          },
