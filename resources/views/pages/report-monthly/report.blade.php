...
<div class="page-inner">
    ...
    >
      <div class="d-flex align-items-center gap-4">
        ...
        <h6 class="op-7 mb-2">Report / Monthly Report</h6>
       
        </div> 
     
      <div class="ms-md-auto py-2 py-md-0">
        @can('add_monthly_report')
        <a href="{{ route('report-monthly-tambah') }}"  class="btn btn-primary btn-round">Tambah</a>
        @endcan
       </div>
    </div>
    <div class="row">
...
function viewDelete(param){
  $(".modal-content").html("");
$.ajax({
  url: "{{ route('report-monthly-delete', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
    type: "GET",
    headers: {
...
function viewShare(param){
  $(".modal-content").html("");
$.ajax({
  url: "{{ route('report-monthly-share', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
    type: "GET",
    headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    },
...
function viewPdf(param){
  $(".modal-content").html("");
  $.ajax({
    url: "{{ route('report-monthly-pdf', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
      type: "GET",
      headers: {
          'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
      },
...
function viewHistory(param){
  $(".modal-content").html("");
$.ajax({
  url: "{{ route('report-monthly-history', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
    type: "GET",
    headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    },
