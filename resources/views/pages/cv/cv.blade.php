    ...
    <div class="page-inner">
        <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row  pb-2">
            ...
            <div class="ms-md-auto py-2 py-md-0">
                @can('add_cv')
                    <a href="{{ route('cv-tambah') }}" class="btn btn-primary btn-round">Add</a>
                @endcan
            </div>
        </div>
        <div class="row">
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-bordered" id="myTable">
                                <thead>
                                    <tr>
                                        <th class="bg-th">Photo</th>
                                        <th class="bg-th">Name</th>
                                        <th class="bg-th">Education</th>
                                        <th class="bg-th">Faculty</th>
                                        <th class="bg-th">University/Institute/Academy</th>
                                        <th class="bg-th">Position</th>
...
@push('bottom')
    ...
    <script>
        function viewDelete(param) {
            $(".modal-content").html("");
            $.ajax({
                url: "{{ route('cv-delete', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
                type: "GET",
                headers: {
        ...
        function viewShare(param) {
            $(".modal-content").html("");
            $.ajax({
                url: "{{ route('cv-share', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
                type: "GET",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
        ...
        function viewPdf(param) {
            $(".modal-content").html("");
            $.ajax({
                url: "{{ route('cv-pdf', ':id') }}".replace(':id', param), // Ganti dengan route yang sesuai
                type: "GET",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
