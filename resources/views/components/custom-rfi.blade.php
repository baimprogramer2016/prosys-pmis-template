@can('view_rfi_in_quality_management')

    @foreach ($sidebar_rfi_in_quality_management as $item_sidebar)
        <li class="nav-item">
                ...
                <ul class="nav nav-collapse">
                    @foreach ($item_sidebar->r_child as $item_sub_sidebar)
                        <li class="nav-item-custom  {{ request('tab') == $item_sub_sidebar->tab ? 'active' : '' }}">
                            <a
                                href="{{ route('custom-rfi', ['tab' => $item_sub_sidebar->tab, 'icon' => $item_sidebar->icon]) }}">
