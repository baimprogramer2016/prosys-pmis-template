@can('view_invoice_record')
@foreach ($sidebar_invoice_record as $item_sidebar)
    <li class="nav-item">
            ...
            <ul class="nav nav-collapse">
                @foreach($item_sidebar->r_child as $item_sub_sidebar)
                    <li class="nav-item-custom  {{ request('tab') == $item_sub_sidebar->tab ? 'active' : '' }}">
                        <a href="{{ route('custom-invoice-record', ['tab' => $item_sub_sidebar->tab,'icon' => $item_sidebar->icon]) }}">
